    ...
    <div className="h-screen gap-5 bg-gradient-to-r p-6 from-cyan-500 to-blue-500 flex flex-col items-center">
      ...
      <div className="flex space-x-3">
        <input
          ...
          className="p-2 w-96 rounded-lg text-lg"
          placeholder="Enter Keyword"
        />
        <button
          onClick={fetchQuote}
          className="bg-pink-400 text-lg p-1 pl-3 pr-3 text-white font-bold rounded-lg"
        >
          Submit
        </button>
      </div>
      ...
      <div className=" border flex justify-end items-center p-6 h-[100vh] w-2/5 rounded-3xl relative bg-[url('https://i.pinimg.com/564x/fa/79/42/fa7942a46ef8a06f6aabbbd77d55f8f1.jpg')] bg-no-repeat bg-cover bg-blend-multiply">
        <div className="h-56 text-3xl font-medium italic font-cursive  w-[60%] flex justify-center items-right  ">
          <p className={error && "text-red-600"}>{loading ? "Loading...": error ? `Opps 😲 ! ${response}` : response}</p>
        </div>
      </div>
    </div>
  );
